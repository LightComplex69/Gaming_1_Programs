...
boolean containsLetter(char key)
{
  for (int i = 0; i < charlist.length; i++)
  {
    if (charlist[i] == key)
    {
      display = key + " was found " + characterCount(key) + " times.";
      return true;
    }
  }
  display = "Key not found. Try another key.";
  return false;
}

int characterCount(char key)
{
  int count = 0;
